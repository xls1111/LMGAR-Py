...
# ========== 创建 Result 文件夹 ==========
result_folder = r'D:\文件\paper2\DATA\4-1\1'
...
# ---------------------------------------------------------------------------------------------------------------------#
precipFilename = r"D:\program\LGAR-Py-LGAR-Py_public\forcing_data_files\synth_1\forcing_data_resampled_synth_1.csv"
...
num_layers = 3
# 第一层: loam
theta_r_layer_0 = 0.078
theta_s_layer_0 = 0.43
K_s_layer_0 = 31.2
alpha_layer_0 = 0.0036
n_layer_0 = 1.56
...
# 第二层: clay loam
theta_r_layer_1 = 0.095
theta_s_layer_1 = 0.41
K_s_layer_1 = 2.6
alpha_layer_1 = 0.0019
n_layer_1 = 1.31
...
# 第三层: silty clay loam
theta_r_layer_2 = 0.089
theta_s_layer_2 = 0.43
K_s_layer_2 = 0.7
alpha_layer_2 = 0.0010
n_layer_2 = 1.23
...
try:
    LGAR_output = pd.read_pickle(r"D:\program\LGAR-Py-LGAR-Py_public\outputs\output_synth_1.pkl")
    ...
    print('路径: D:\\program\\LGAR-Py\\outputs\\output_synth_1.pkl')
    lgar_has_data = False
    LGAR_output = None
...
HYDRUS_output = pd.read_fwf(r"D:\program\LGAR-Py-LGAR-Py_public\HYDRUS_files\synthetic_test_1_HYDRUS\T_Level.txt",
...
axins1.plot(times_to_plot[zoom_start:zoom_end],
           HYDRUS_output['sum(Infil'].iloc[zoom_start:zoom_end],
           linewidth=3, color='#0066CC', linestyle='-', alpha=0.8)
axins1.plot(times_to_plot[zoom_start:zoom_end],
           MLGAR_output['cummInfil'].iloc[zoom_start:zoom_end],
...
ax1.set_title('(a) Cumulative Infiltration Comparison', fontsize=16,
              fontweight='bold', loc='left', pad=12)  # 修改pad从20到12
...
# 缩放区域 (5-7.5小时)
zoom_start2 = np.where(times_to_plot >= 5)[0][0] if len(np.where(times_to_plot >= 5)[0]) > 0 else 0
zoom_end2 = np.where(times_to_plot <= 7.5)[0][-1] if len(np.where(times_to_plot <= 7.5)[0]) > 0 else min_plt_len
...
ax2.set_title('(b) Cumulative Runoff Comparison', fontsize=16,
              fontweight='bold', loc='left', pad=12)  # 修改pad从20到12

# 调整子图间距 - 关键修改：给顶部留更多空间
plt.subplots_adjust(left=0.08, right=0.95, top=0.96, bottom=0.05, hspace=0.28)

# 保存图片
plt.savefig(os.path.join(result_folder, 'infiltration_runoff_combined_with_zoom.png'),
            dpi=300)
plt.close()
