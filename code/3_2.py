...
# ========== 创建 Result 文件夹 ==========
result_folder = r'D:\文件\paper2\DATA\4-1\3'
...
# ---------------------------------------------------------------------------------------------------------------------#
precipFilename = r"D:\program\LGAR-Py\forcing_data_files\synth_3\forcing_data_resampled_synth_3.csv"
...
num_layers = 3
# 第一层: loam
theta_r_layer_0 = 0.057
theta_s_layer_0 = 0.41
K_s_layer_0 = 145.9164
alpha_layer_0 = 0.0124
n_layer_0 = 2.28
...
# 第二层: clay loam
theta_r_layer_1 = 0.078
theta_s_layer_1 = 0.43
K_s_layer_1 = 10.4
alpha_layer_1 = 0.0036
n_layer_1 = 1.56
...
try:
    LGAR_output = pd.read_pickle(r'D:\program\LGAR-Py\outputs\output_synth_3.pkl')
...
HYDRUS_output = pd.read_fwf(r'D:\program\LGAR-Py\HYDRUS_files\synthetic_test_3_HYDRUS\T_Level.txt',
...
ax.plot(times_to_plot[:len(MLGAR_output)], MLGAR_output['cummInfil'],
        linewidth=3, color='#FF8800', label='LMGAR - Cumulative Infiltration', linestyle='--',
...
ax.plot(times_to_plot[:len(MLGAR_output)], MLGAR_output['runoff'],
        linewidth=3, color='#9900FF', label='LMGAR - Cumulative Runoff', linestyle='--',
...
# ========== Combined Plot: Infiltration and Runoff with Local Zoom ==========
from mpl_toolkits.axes_grid1.inset_locator import inset_axes, mark_inset

# 创建包含两个子图的图像（2行1列）
fig, axes = plt.subplots(2, 1, figsize=(16, 18))

# ==================== 子图 (a): 累积渗流 Cumulative Infiltration ====================
ax1 = axes[0]
